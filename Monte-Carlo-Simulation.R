...
# MIXTURES
## ------------------------
##  Normal Contaminated 5% lambda=3 MIXTURES
# E(X) = mu
# VarX = sigma^2
## ------------------------
...
#########################################################################################
## ------------------------
##  Normal Contaminated 10% lambda=3 MIXTURES
# E(X) = mu
# VarX = sigma^2
## ------------------------
...
#########################################################################################
## ------------------------
##  Normal Contaminated 15% lambda=3 MIXTURES
# E(X) = mu
# VarX = sigma^2
## ------------------------
...
#########################################################################################
## ------------------------
##  Normal Contaminated 20% lambda=3 MIXTURES
# E(X) = mu
# VarX = sigma^2
## ------------------------
...
#########################################################################################
# Really contaminated with Heavy tails
# ------------------------
#  Normal Contaminated with 5% of Heavy tails (t-distribution with 2.5df)
# E(X) = mu
# VarX = sigma^2
## ------------------------
...
save(params, file="Normal-Heavy-5-params.Rdata")
...
for(i in 1:NROW(params)){
  ...
  res <- parallel:::mclapply(1:B, function(i){
    # x <- (1-(20/100))*rnorm(n = r[1], mean = r[2], s = r[3])+
    #   (20/100)*rnorm(n = r[1], mean = r[2], s = sqrt(3)*r[3])
    x <- c(rnorm(n=r[1]*0.95, mean = r[2], s = r[3]), rt(n=r[1]*0.05, df = 2.5) )    
    cvs(x)
  }, mc.cores = 4)
  res <- do.call(rbind, res)
  L[i, ] <- colMeans((res-r[4])^2, na.rm = TRUE)
...
save(out, file="Normal-Heavy-5-Sim-Wide.Rdata")
load("Normal-Heavy-5-Sim-Wide.Rdata")
data_long <- gather(out, Estimator, gamma, CQV7:CQVMnAD, factor_key=TRUE)
data_long$n=as.factor(data_long$n)
# data_long$m=as.factor(data_long$m)
# data_long$p=as.factor(format(data_long$p, digits=2)  )
...
save(data_long, file="Normal-Heavy-5-Sim-Long.Rdata")

Normal.Heavy5 = data_long %>% group_by(n, Estimator) %>% summarise(med=median(lgamma))
save(Normal.Heavy5, file="Normal.Heavy-5-Table.Rdata")


# v.1 = plot.CV.1(data=data_long, name.distribution="Normal-20")
# v.2 = plot.CV.2(data=data_long, name.distribution="Normal-20")

load("Normal-Heavy-5-Sim-Long.Rdata")
v.3 = plot.CV.3(data=data_long, name.distribution="Normal-Heavy-5")
#########################################################################################
...
# Really contaminated with Heavy tails
# ------------------------
#  Normal Contaminated with 10% of Heavy tails (t-distribution with 2.5df)
# E(X) = mu
# VarX = sigma^2
## ------------------------
...
save(params, file="Normal-Heavy-10-params.Rdata")
...
for(i in 1:NROW(params)){
  ...
  res <- parallel:::mclapply(1:B, function(i){
    # x <- (1-(20/100))*rnorm(n = r[1], mean = r[2], s = r[3])+
    #   (20/100)*rnorm(n = r[1], mean = r[2], s = sqrt(3)*r[3])
    x <- c(rnorm(n=r[1]*0.9, mean = r[2], s = r[3]), rt(n=r[1]*0.1, df = 2.5) )    
    cvs(x)
  }, mc.cores = 4)
  res <- do.call(rbind, res)
  L[i, ] <- colMeans((res-r[4])^2, na.rm = TRUE)
...
save(out, file="Normal-Heavy-10-Sim-Wide.Rdata")
load("Normal-Heavy-10-Sim-Wide.Rdata")
data_long <- gather(out, Estimator, gamma, CQV7:CQVMnAD, factor_key=TRUE)
data_long$n=as.factor(data_long$n)
# data_long$m=as.factor(data_long$m)
# data_long$p=as.factor(format(data_long$p, digits=2)  )
...
save(data_long, file="Normal-Heavy-10-Sim-Long.Rdata")

Normal.Heavy10 = data_long %>% group_by(n, Estimator) %>% summarise(med=median(lgamma))
save(Normal.Heavy10, file="Normal.Heavy-10-Table.Rdata")


# v.1 = plot.CV.1(data=data_long, name.distribution="Normal-20")
# v.2 = plot.CV.2(data=data_long, name.distribution="Normal-20")

load("Normal-Heavy-10-Sim-Long.Rdata")
v.3 = plot.CV.3(data=data_long, name.distribution="Normal-Heavy-10")
#########################################################################################
...
# Really contaminated with Heavy tails
# ------------------------
#  Normal Contaminated with 15% of Heavy tails (t-distribution with 2.5df)
# E(X) = mu
# VarX = sigma^2
## ------------------------
...
save(params, file="Normal-Heavy-15-params.Rdata")
...
for(i in 1:NROW(params)){
  ...
  res <- parallel:::mclapply(1:B, function(i){
    # x <- (1-(20/100))*rnorm(n = r[1], mean = r[2], s = r[3])+
    #   (20/100)*rnorm(n = r[1], mean = r[2], s = sqrt(3)*r[3])
    x <- c(rnorm(n=r[1]*0.85, mean = r[2], s = r[3]), rt(n=r[1]*0.15, df = 2.5) )    
    cvs(x)
  }, mc.cores = 4)
  res <- do.call(rbind, res)
  L[i, ] <- colMeans((res-r[4])^2, na.rm = TRUE)
...
save(out, file="Normal-Heavy-15-Sim-Wide.Rdata")
load("Normal-Heavy-15-Sim-Wide.Rdata")
data_long <- gather(out, Estimator, gamma, CQV7:CQVMnAD, factor_key=TRUE)
data_long$n=as.factor(data_long$n)
# data_long$m=as.factor(data_long$m)
# data_long$p=as.factor(format(data_long$p, digits=2)  )
...
save(data_long, file="Normal-Heavy-15-Sim-Long.Rdata")

Normal.Heavy15 = data_long %>% group_by(n, Estimator) %>% summarise(med=median(lgamma))
save(Normal.Heavy15, file="Normal.Heavy-15-Table.Rdata")


# v.1 = plot.CV.1(data=data_long, name.distribution="Normal-20")
# v.2 = plot.CV.2(data=data_long, name.distribution="Normal-20")

load("Normal-Heavy-15-Sim-Long.Rdata")
v.3 = plot.CV.3(data=data_long, name.distribution="Normal-Heavy-15")
#########################################################################################
...
# Really contaminated with Heavy tails
# ------------------------
#  Normal Contaminated with 20% of Heavy tails (t-distribution with 2.5df)
# E(X) = mu
# VarX = sigma^2
## ------------------------
...
save(params, file="Normal-Heavy-20-params.Rdata")
...
for(i in 1:NROW(params)){
  ...
  res <- parallel:::mclapply(1:B, function(i){
    # x <- (1-(20/100))*rnorm(n = r[1], mean = r[2], s = r[3])+
    #   (20/100)*rnorm(n = r[1], mean = r[2], s = sqrt(3)*r[3])
    x <- c(rnorm(n=r[1]*0.8, mean = r[2], s = r[3]), rt(n=r[1]*0.2, df = 2.5) )    
    cvs(x)
  }, mc.cores = 4)
  res <- do.call(rbind, res)
  L[i, ] <- colMeans((res-r[4])^2, na.rm = TRUE)
...
save(out, file="Normal-Heavy-20-Sim-Wide.Rdata")
load("Normal-Heavy-20-Sim-Wide.Rdata")
data_long <- gather(out, Estimator, gamma, CQV7:CQVMnAD, factor_key=TRUE)
data_long$n=as.factor(data_long$n)
# data_long$m=as.factor(data_long$m)
# data_long$p=as.factor(format(data_long$p, digits=2)  )
...
save(data_long, file="Normal-Heavy-20-Sim-Long.Rdata")

Normal.Heavy20 = data_long %>% group_by(n, Estimator) %>% summarise(med=median(lgamma))
save(Normal.Heavy20, file="Normal.Heavy-20-Table.Rdata")


# v.1 = plot.CV.1(data=data_long, name.distribution="Normal-20")
# v.2 = plot.CV.2(data=data_long, name.distribution="Normal-20")

load("Normal-Heavy-20-Sim-Long.Rdata")
v.3 = plot.CV.3(data=data_long, name.distribution="Normal-Heavy-20")
#########################################################################################
...
# Heavy-Tails
load("Normal.Heavy-5-Table.Rdata")
load("Normal.Heavy-10-Table.Rdata")
load("Normal.Heavy-15-Table.Rdata")
load("Normal.Heavy-20-Table.Rdata")
...
# Heavy tails
# Place supergroups, more_bands, and more_artists into a list
tt <- as.data.frame(list(Normal, Normal.Heavy5, Normal.Heavy10, Normal.Heavy15, Normal.Heavy20) %>%
                      reduce(left_join, by = c("n" = "n","Estimator" = "Estimator")))

sink(file="tabela-Heavy.txt")
knitr::kable(
  tt, caption = 'Heavy', format="latex", digits=2,
  booktabs = TRUE
)
sink()
#---------------------------------------------------------------------------
